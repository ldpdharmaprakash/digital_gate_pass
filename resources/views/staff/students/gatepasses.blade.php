...
@section('title', 'Student Gatepasses')

@section('content')
<div x-data="studentGatepasses()">
    <!-- Page Header -->
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ $student->user->name }} - Gatepasses</h1>
            <p class="text-gray-600 mt-2">View all gatepass requests for {{ $student->register_number }}</p>
        </div>
        <div class="flex items-center space-x-4">
            <a href="{{ route('staff.students.index') }}" class="text-blue-600 hover:text-blue-900">
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0 h-16 w-16">
                    <div class="h-16 w-16 rounded-full bg-blue-500 flex items-center justify-center">
                        <span class="text-white text-xl font-medium">{{ substr($student->user->name, 0, 1) }}</span>
                    </div>
                </div>
                <div class="ml-6">
                    <h2 class="text-xl font-semibold text-gray-900">{{ $student->user->name }}</h2>
                    <div class="mt-1 space-y-1">
                        <p class="text-sm text-gray-600">Register Number: {{ $student->register_number }}</p>
                        <p class="text-sm text-gray-600">Department: {{ $student->department->name }}</p>
                        <p class="text-sm text-gray-600">Semester: {{ $student->semester }}</p>
                        <p class="text-sm text-gray-600">Email: {{ $student->user->email }}</p>
                        <p class="text-sm text-gray-600">Phone: {{ $student->user->phone }}</p>
                    </div>
                </div>
            </div>
            <div class="text-right">
                @if($student->hosteller === 'yes')
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">Hosteller</span>
                @else
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">Day Scholar</span>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $gatepasses->where('status', 'pending')->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $gatepasses->where('status', 'final_approved')->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $gatepasses->whereIn('status', ['staff_rejected', 'hod_rejected', 'warden_rejected'])->count() }}</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @forelse($gatepasses as $gatepass)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @elseif(in_array($gatepass->status, ['staff_rejected', 'hod_rejected', 'warden_rejected']))
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $gatepass->created_at->format('M d, Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if($gatepass->status === 'pending' && $gatepass->canBeApprovedByStaff())
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="text-sm text-gray-500">This student hasn't submitted any gatepass requests yet.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
